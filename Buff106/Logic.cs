...
namespace Buff106
{
	partial class EventHandler
	{
		...
		private IEnumerator<float> CheckPlayerTouch(Player scp106)
		{
			...
			int pID = scp106.PlayerId;
			while (scp106 != null &&
				scp106.GetHealth() > 0 &&
				scp106.GetHealth() <= touchHealth &&
				isRoundStarted &&
				scp106.TeamRole.Role == Role.SCP_106 &&
				scpList.ContainsKey(pID) &&
				scpList[pID])
			{
				IEnumerable<Player> pList = instance.Server.GetPlayers().Where(x => x.PlayerId != scp106.PlayerId && x.TeamRole.Team != Smod2.API.Team.SCP);
